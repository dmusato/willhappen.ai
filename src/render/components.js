...
export function dial(p, { compact = false } = {}) {
  ...
  <div class="dial-track" style="--lanes:${laneCount(cells)}">
    ...
    ${lanes(cells).map(({ m, cell, lane }) =>
      // Not title=": the browser's own tooltip waits about a second, renders in
      // the OS style, and never appears on touch at all. This is a sibling
      // element the dot reveals, so it is instant and looks like the page.
      //
      // `lane` lifts a dot clear of the ones it would sit on top of. Agreement
      // is common — six models inside three points is a normal result — and
      // stacked at one spot the dial stops saying anything at all.
      `<span class="dial-dot" style="--at:${cell.prob};--c:${m.color};--lane:${lane};--tip-x:${tipAnchor(cell.prob)}" tabindex="0" role="button"
        aria-label="${h(m.name)}, ${cell.prob} percent"><b class="dial-tip">${h(m.name)} <i>${cell.prob}%</i></b></span>`).join("")}
  </div>
  ${compact ? "" : `<div class="dial-axis"><span>0</span><span>impossible ← → certain</span><span>100</span></div>`}
</div>`;
}

// A tip centred on a dot near either end of the track runs off the screen, and
// on a phone that covers most confident answers. CSS cannot branch on a custom
// property's value, so the anchor is decided here.
const tipAnchor = (prob) => (prob <= 14 ? "0%" : prob >= 86 ? "-100%" : "-50%");

// Gives each dot a row so that dots which would overlap sit above one another
// instead. A dot is about 15px and the track is a few hundred, so anything
// inside roughly seven points collides on a phone — which is most of the
// archive, since the usual result is six models broadly agreeing.
function lanes(cells, minGap = 7) {
  const sorted = [...cells].sort((a, b) => a.cell.prob - b.cell.prob);
  const lastInLane = [];
  return sorted.map((entry) => {
    let lane = lastInLane.findIndex((prob) => entry.cell.prob - prob >= minGap);
    if (lane === -1) lane = lastInLane.length;
    lastInLane[lane] = entry.cell.prob;
    return { ...entry, lane };
  });
}

// How many rows the stack actually needs, so the track can centre it rather
// than letting the top row escape upwards.
const laneCount = (cells) => Math.max(1, ...lanes(cells).map((x) => x.lane + 1));
