...
<article class="detail">
  <nav class="crumbs mono"><a href="/timeline">Timeline</a> ${t ? `<span>/</span> <a href="/topics/${h(p.topic)}">${h(t.name)}</a>` : ""} <span>/</span> <span>${h(hz?.label || p.horizon)}</span></nav>
...
${related.length ? section("More in this thread", rows(related), { kicker: "RELATED", id: "related" }) : ""}`;
  ...
  return shell({
    ...
    jsonLd: predictionSchema(p, site, canonical),
  });
}
...
// accurate and what search engines read for this kind of content.
function predictionSchema(p, site, canonical) {
  const org = { "@type": "Organization", name: "WillHappen.ai", url: site };
  const published = p.question_generated_at || p.created_at;
  const image = `${site}/og/${encodeURIComponent(p.id)}.png`;
  ...
  if (settled) {
    return {
      ...
      url: canonical,
      datePublished: (p.verdict_at || published || "").slice(0, 10),
      author: org,
      claimReviewed: p.headline.slice(0, 110),
      reviewRating: {
        "@type": "Rating",
        // A binary outcome, stated on the scale Google expects: 5 means the
        // claim held, 1 means it did not.
        ratingValue: p.verdict ? 5 : 1,
        bestRating: 5,
        worstRating: 1,
        alternateName: p.verdict ? "True" : "False",
      },
      itemReviewed: {
        "@type": "Claim",
        appearance: p.source_url ? [{ "@type": "CreativeWork", url: p.source_url }] : undefined,
        firstAppearance: { "@type": "CreativeWork", url: canonical },
      },
  ...
  const answers = PANEL
    ...
    .filter(({ cell }) => typeof cell?.prob === "number");
  ...
  return {
    ...
    mainEntity: {
      ...
      acceptedAnswer: {
        "@type": "Answer",
        text: `${p.consensus_prob}% — the median of ${answers.length} frontier models, resolving by ${p.resolves_by}.`,
        url: canonical,
        author: org,
        datePublished: published,
      },
...
// Says in words what the dial says in pixels. Every sentence is derived from
// the record, so this costs no model call and applies to the whole archive at
// once — and a page that states its own numbers in prose is both a better read
// and a page a search engine can tell apart from the next one.
function reading(p, answered) {
  const s = [];
  const c = p.consensus_prob;
  const strength =
    c >= 85 ? "treats this as close to settled" :
    c >= 65 ? "leans yes" :
    c >= 50 ? "is barely on the yes side" :
    c >= 35 ? "leans no" :
    c >= 15 ? "thinks this is unlikely" :
              "has all but ruled it out";
  s.push(`Across ${answered} frontier model${answered === 1 ? "" : "s"} answering independently, the panel ${strength}: the median is <b>${c}%</b>.`);

  const cells = PANEL.map((m) => ({ m, cell: p.models?.[m.key] })).filter((x) => typeof x.cell?.prob === "number");
  if (cells.length >= 2 && p.spread != null) {
    const top = cells.reduce((a, b) => (b.cell.prob > a.cell.prob ? b : a));
    const bot = cells.reduce((a, b) => (b.cell.prob < a.cell.prob ? b : a));
    s.push(p.spread >= 30
      ? `They do not agree: ${h(top.m.name)} puts it at ${top.cell.prob}% and ${h(bot.m.name)} at ${bot.cell.prob}%, ${p.spread} points apart. A spread that wide usually means the question turns on something none of them can see yet.`
      : `They broadly agree, ${p.spread} points between ${h(top.m.name)} at ${top.cell.prob}% and ${h(bot.m.name)} at ${bot.cell.prob}%.`);
  }

  if (typeof p.market?.prob === "number") {
    const edge = c - p.market.prob;
    const gap = Math.abs(edge);
    s.push(gap <= 3
      ? `Traders risking real money on ${h(sourceLabel(p))} land in the same place, at ${p.market.prob}%.`
      : `Money disagrees. The market sits at ${p.market.prob}%, so the models are <b>${gap} points ${edge > 0 ? "more" : "less"}</b> confident than people with cash on the outcome${gap >= 20 ? " — one of the wider gaps in the archive" : ""}.`);
  }

  if (p.verdict === true || p.verdict === false) {
    s.push(`It ${p.verdict ? "happened" : "did not happen"}. Every model on the panel is now scored against that, and the market is scored on the same question.`);
  } else {
    s.push(`Nothing is settled until ${h(fmtDate(p.resolves_by))}, when the outcome is checked against sources and every number above gets a permanent score.`);
  }

  return `<div class="detail-reading">${s.map((line) => `<p>${line}</p>`).join("")}</div>`;
}
...
${stats.map(({ t, n, resolved, hits, avg, latest }) => `  <a class="topic-card" href="/topics/${h(t.id)}" style="--hue:${t.hue}">
...
// ── one subject ─────────────────────────────────────────────
// A page per topic, not /timeline?topic=…. A query string is a weaker target
// for search, cannot carry its own title and description, and never appeared in
// the sitemap at all — so the 24 subjects this archive is organised around had
// no landing page between the index and an individual forecast.
export function topicPage({ site, t, list, total }) {
  const settled = list.filter((p) => p.verdict === true || p.verdict === false);
  const hits = settled.filter((p) => p.verdict === true).length;
  const avg = list.length ? Math.round(list.reduce((a, p) => a + (p.consensus_prob || 0), 0) / list.length) : null;
  // Worth pointing at only when it is not already the row directly underneath —
  // on a small topic the soonest deadline is usually also the newest question,
  // and naming it twice in 40px of page is noise.
  const nextUp = [...list]
    .filter((p) => p.verdict === null || p.verdict === undefined)
    .sort((a, b) => String(a.resolves_by).localeCompare(String(b.resolves_by)))[0] || null;
  const soonest = nextUp && nextUp.id !== list[0]?.id ? nextUp : null;

  // Sentence-initial numerals read badly and "1 have" reads worse, so the two
  // counts get their own wording rather than a chain of ternaries inline.
  const graded = !settled.length ? ""
    : settled.length === 1 ? " One has since passed its deadline and been graded against cited sources."
    : ` Of those, ${settled.length} have passed their deadlines and been graded against cited sources.`;

  const body = `
<section class="page-head topic-head" style="--hue:${t.hue}">
  <nav class="crumbs mono"><a href="/topics">Topics</a> <span>/</span>
    <span class="crumb-now"><i aria-hidden="true">${h(t.icon)}</i> ${h(t.name)}</span></nav>
  <h1><i>${h(t.name)}</i> predictions</h1>
  <p class="lede">${total} dated question${total === 1 ? "" : "s"} on ${h(t.hint)} — each one answered
  independently by six frontier models, none of them shown the market price.${graded}</p>
  <dl class="stats">
    <div><dt>forecasts</dt><dd>${total}</dd></div>
    ${avg === null ? "" : `<div><dt>average odds</dt><dd>${avg}%</dd></div>`}
    <div><dt>settled</dt><dd>${settled.length}</dd></div>
    ${settled.length ? `<div><dt>came true</dt><dd>${Math.round((hits / settled.length) * 100)}%</dd></div>` : ""}
  </dl>
  ${soonest ? `<p class="next-up"><span class="mono">Next deadline ·
    <time datetime="${h(soonest.resolves_by)}">${h(fmtDate(soonest.resolves_by))}</time></span>
    <a href="/p/${h(soonest.id)}">${h(soonest.headline)}</a></p>` : ""}
</section>

${rows(list)}

${section("Other subjects", `<div class="chips big">${TOPICS.filter((o) => o.id !== t.id)
    .map((o) => `<a class="chip-link" href="/topics/${h(o.id)}">${h(o.icon)} ${h(o.name)}</a>`).join("")}</div>`,
    { kicker: "KEEP READING" })}`;

  return shell({
    site, page: "topic", body,
    title: `${t.name} predictions — what six AI models expect | WillHappen.ai`,
    description: `${total} forecasts on ${t.hint}. Six frontier AI models, each answering independently, benchmarked against prediction-market prices and graded against sources.`,
    canonical: `${site}/topics/${t.id}`,
    jsonLd: {
      "@context": "https://schema.org", "@type": "CollectionPage",
      name: `${t.name} predictions`,
      description: `AI consensus forecasts on ${t.hint}.`,
      url: `${site}/topics/${t.id}`,
      isPartOf: { "@type": "WebSite", name: "WillHappen.ai", url: site },
      mainEntity: {
        "@type": "ItemList",
        numberOfItems: list.length,
        itemListElement: list.slice(0, 25).map((p, i) => ({
...
// ── terms ───────────────────────────────────────────────────
// Written plainly on purpose. A wall of boilerplate nobody reads protects less
...
<section class="page-head">
  ...
  <p class="lede">Short version: this is an automated experiment that publishes machine-generated
  guesses about the future. Some of them are wrong. Do not make decisions that matter on the strength
  of a number you read here.</p>
</section>

<div class="prose">
  <h2>This is information, not advice</h2>
  <p>Nothing on this site is financial, investment, trading, betting, legal, tax, medical or
  professional advice of any kind, and nothing here is a recommendation or solicitation to buy, sell
  or stake anything. The probabilities are produced by language models answering a prompt. They are
  not research, they are not a forecast from anyone qualified to give one, and no human reviews them
  before they publish.</p>
  <p>If a number here would change what you do with your money, your vote or your health, treat that
  as a sign to go and read a primary source instead.</p>

  <h2>Accuracy is not promised</h2>
  <p>The whole point of the scoreboard is that the models are often wrong, and we publish the misses
  as prominently as the hits. Questions are rewritten by a model, which can subtly change what is
  being asked. Outcomes are decided by exchange settlement, by a panel of models reading cited
  sources, or by hand — and each of those can be mistaken.</p>
  <p>The site is provided as is, without warranty of any kind, express or implied, including
  merchantability, fitness for a particular purpose and non-infringement. To the fullest extent the
  law allows, the maintainers are not liable for any loss or damage arising from use of this site or
  reliance on anything published here.</p>
  <p>If you think a published outcome is wrong,
  <a href="https://github.com/dmusato/willhappen.ai/issues/new?template=report-outcome.yml" rel="noopener" target="_blank">say so in public</a>.
  Corrections are part of the design, not an embarrassment.</p>

  <h2>Not affiliated with anyone named here</h2>
  <p>Market prices are read from the public APIs of Polymarket and Kalshi. Model answers are obtained
  through OpenRouter. This project is independent of those services and of every AI lab whose model
  appears on the panel. No endorsement in either direction is implied, and all trademarks belong to
  their owners.</p>
  <p>Prediction markets are regulated differently from one country to the next, and in some places
  they are not available to you at all. Whether you may use them is your responsibility to determine,
  not ours. We take no position on it and link to them only as the source of a price.</p>

  <h2>Links and third parties</h2>
  <p>Pages link out to news sources and exchanges. We do not control that content and are not
  ...
  <p>This one. The source is MIT licensed, so other people may be running their own copies; those are
  their sites, with their questions and their answers, and nothing here speaks for them.</p>

  <h2>Changes</h2>
  <p>These terms may change; the current version is always the one on this page, and its history is in
  the public repository like everything else.</p>

  <p class="muted">Written to be read rather than to be airtight. It is not legal advice and has not
...
export function termsPage({ site }) {
  ...
  return shell({
    ...
    title: "Terms and disclaimer | WillHappen.ai",
    description: "WillHappen.ai publishes machine-generated probability estimates for information only. No advice, no warranty, no affiliation with the exchanges or labs named on the site.",
...
<div class="prose">
  ...
  <h2>Why forecasts arrive in batches</h2>
  <p>The machine wakes once an hour and does one slice of the work: harvest a few questions, put a couple to the
  panel, check what has come due. Nothing is queued up and released on a schedule — a forecast appears when its
  turn comes round. Between slices it keeps re-pricing open markets and recomputing the scoreboard, which needs
  no model at all.</p>

  <h2>Check it yourself</h2>
  <p>Every claim on this page is a claim about code you can read. The prompts, the median, the Brier maths and the
  three-tier resolver are all in one MIT-licensed repository, and the forecasts are on a public API.</p>
  <p><a class="btn" href="https://github.com/dmusato/willhappen.ai" rel="noopener" target="_blank">Read the source</a></p>
