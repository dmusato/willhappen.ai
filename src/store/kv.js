// The archive's keyspace, in one place, so the shape of the storage is
// documented by the code that uses it rather than by a table that drifts.
//
//   prediction:{id}      full record
//   predictions:index    compact listing
...
//   ledger:{YYYY-MM-DD}  spend + counters for one UTC day  (40d)
//   review:queue         verdicts the resolver was not confident enough to publish
//   resolve:checked      {id: lastCheckedAt}, so a slow market is not re-examined every run
//   cursor:{name}        rotation cursors (news beats, market pages)
//
// Three kinds of key are deliberately NOT here, because nothing outside the one
// feature that owns them ever reads them, and each carries a TTL that only makes
// sense next to the code setting it: votes (votes:tally:{id}, votes:by:{id}:{fp})
// in handlers/vote.js, rate limits (rl:*) in the handler doing the limiting, and
// posted-checkpoints (social:{net}:{id}) in social/index.js.

const INDEX_KEY = "predictions:index";
const QUEUE_KEY = "queue:questions";
...
// Two readers, deliberately. A caller deciding whether to seed has to tell an
// absent index from an empty one — collapsing them re-seeds KV on every request
// to a site that genuinely has no forecasts yet.
export const readIndex = (env) => env.WH_KV.get(INDEX_KEY, "json");
export const writeIndexDoc = (env, doc) => env.WH_KV.put(INDEX_KEY, JSON.stringify(doc));

export async function getIndex(env) {
  return (await readIndex(env)) || { generated_at: null, predictions: [] };
}
...
export async function writeIndex(env, records) {
  ...
  const predictions = [...rows.values()].sort(byAsked);
  await writeIndexDoc(env, { generated_at: new Date().toISOString(), predictions });
  return predictions.length;
}

// The index is ordered by when a question was asked, and stays put when the
// answer arrives. Not the same as the pages' byNewest, which floats a forecast
// back up on the day it is graded — two orders, so two names.
const byAsked = (a, b) =>
...
const putQueue = (env, q) => env.WH_KV.put(QUEUE_KEY, JSON.stringify(q.slice(0, MAX_QUEUE)));
...
const dailyBudget = (env) => Math.max(0, Number(env.DAILY_BUDGET_USD ?? 0.7) || 0);
...
// ── leaderboard ──────────────────────────────────────────────
export const readLeaderboard = async (env) => (await env.WH_KV.get(LEADERBOARD_KEY, "json")) || null;
export const writeLeaderboard = (env, board) => env.WH_KV.put(LEADERBOARD_KEY, JSON.stringify(board));
