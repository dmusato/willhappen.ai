...
// Cloudflare KV rejects any expirationTtl below this rather than rounding up.
const MIN_TTL = 60;
...
export async function handleVote(request, env) {
  ...
  //
  // Scoped to the prediction rather than the whole site: a reader working down
  // the archive votes on several questions in a minute, and that is the
  // behaviour we want, not the one to throttle. Stuffing one question from one
  // address is what this stops.
  const ip = request.headers.get("cf-connecting-ip") || "anon";
  const ipKey = `rl:vote:${ip}:${id}`;
  ...
  tally[verdict] += 1;
  // 60 is KV's floor, and asking for less is not clamped — it throws, which is
  // what turned every single vote into a 500. Nothing on the page said so:
  // the tally simply never moved.
  await env.WH_KV.put(ipKey, "1", { expirationTtl: MIN_TTL });
