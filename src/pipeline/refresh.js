...
export async function refreshMarkets(env, { max = 8 } = {}) {
  ...
  await pool(records, 4, async (p) => {
    try {
      ...
      // The exchange always answers about its own question. When our headline
      // states the opposite, the crowd's view of ours is the complement — and
      // without this the hourly re-price would quietly undo the correction made
      // at harvest and put the inverted number back on the page.
      const liveProb = p.market.inverted ? 100 - live.prob : live.prob;
      ...
      const next = {
        ...
        edge: p.consensus_prob - liveProb,
      };
