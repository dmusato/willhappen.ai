...
`npm run dev` serves the pages; the hourly slice only runs when its cron fires, so trigger
one by hand to watch the loop go round:

```bash
npm run cron      # fires the scheduled handler against the local Worker
```

```bash
npm run markets   # what the exchanges would give you right now, no keys needed
npm test          # unit tests
npm run lint      # syntax, imports and JSON — no dependencies
```

## Configuration

Everything lives in `wrangler.toml`.
...
src/
  ...
  ai/
    ...
    polarity.js      does the rewritten headline still mean the market question?
  ...
  discover/          suggest.js — what people actually search for
  render/            server-side HTML: shell.js · components.js · pages.js · markdown.js
  handlers/          /api/*, share cards, sitemap, RSS, admin, MCP, llms.txt
  social/            postiz.js + direct X / Reddit / Meta adapters
  store/kv.js        the archive keyspace, in one file
...
A resolved record carries `verdict_method` — `exchange`, `jury` or `maintainer` — so
anything built on this can weight an outcome by how it was decided. Open questions have
no verdict and no method.

Sorts: `new`, `soon`, `far`, `contested`, `edge`, `confident`, `likely`, `unlikely`.

### For agents

The archive is meant to be read by software as much as by people, so the same content is
served in the shapes an agent already knows how to ask for.

| Endpoint | Returns |
|---|---|
| `POST /mcp` | MCP server, Streamable HTTP and stateless. Tools: `search_forecasts`, `get_forecast`, `get_scoreboard`, `list_topics`. |
| any page + `.md` | The same page as Markdown — `/p/{id}.md`, `/timeline.md`, `/about.md`. Or send `Accept: text/markdown`. |
| `GET /llms.txt` | What this is, how to read a number here, where the rest lives. |
| `GET /.well-known/mcp.json` | Server card for the above. |
| `GET /.well-known/api-catalog` | RFC 9727 linkset of the JSON endpoints. |
| `GET /.well-known/ai-catalog.json` | ARD capability manifest. |

No key, no session, no rate limit worth mentioning — every tool is a public read.
