...
```
  ...
  ┌──────────────────────────────────────────────────────────────┐
  │ fetch()                                                      │
  │   /         /timeline  /topics  /topics/{id}                 │
  │   /models   /about     /terms   /suggest                     │
  ...
  │   /mcp             MCP server, four read-only tools          │
  │   /llms.txt  /.well-known/{mcp,api-catalog,ai-catalog}       │
  │   {page}.md        any page as Markdown (or Accept:)         │
...
src/
  ...
  discover/
    suggest.js           what people actually search, via autocomplete
  render/
    shell.js             document shell, nav, footer
    components.js        dial, rows, model list, market panel, verdict
    pages.js             one function per route
    markdown.js          the same pages as Markdown, for readers without a browser
  handlers/
    http.js              json(), preflight(), requireAdmin()
    predictions.js       listing + filters + sorts, seed fallback
    og.js                share cards, both variants
    mcp.js               MCP server — four read-only tools over the archive
    agents.js            llms.txt, RFC 9727 api-catalog, ARD manifest
    indexnow.js          tells the search engines a page changed
  ...
  store/kv.js            the archive keyspace — records, index, queue, scores, ledger
...
| `npm run cron` | fire the hourly slice against the local Worker |
| `npm test` | unit tests |
| `npm run lint` | syntax, imports and JSON — zero dependencies |
